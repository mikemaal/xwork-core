 ...
 */
public class XmlConfigurationProvider implements ConfigurationProvider, Serializable {
   ...
   public boolean equals(Object o) {
      ...
      if ((configFileName != null) ? (!configFileName.equals(xmlConfigurationProvider.configFileName))
            ...
            if (childNode instanceof Element) {
               ...
               if ("bean".equals(nodeName)) {
                  ...
                  try {
                     ...
                     if ("true".equals(onlyStatic)) {
                        ...
                        if (containerBuilder.contains(ctype, name)) {
                           ...
                           if (throwExceptionOnDuplicateBeans) {
                              throw new ConfigurationException("Bean type " + ctype + " with the name " + name
                                    + " has already been loaded by " + loc, child);
                           }
                        }
                        // Force loading of class to detect no class def found exceptions
                     ...
                     if (!optional) {
                        throw new ConfigurationException("Unable to load bean: type:" + type + " class:" + impl, ex,
                              childNode);
                     } else {
                        if (LOG.isDebugEnabled()) {
   ...
   protected void addAction(Element actionElement, PackageConfig.Builder packageContext) throws ConfigurationException {
      ...
      if (StringUtils.isEmpty(className)) {
         ...
         if (!verifyAction(className, name, location)) {
            if (LOG.isErrorEnabled())
               LOG.error("Unable to verify action [#0] with class [#1], from [#2]", name, className,
                     location.toString());
            return;
         }
      }
      ...
      ActionConfig actionConfig = new ActionConfig.Builder(packageContext.getName(), name, className)
            .methodName(methodName).addResultConfigs(results).addInterceptors(interceptorList)
            .addExceptionMappings(exceptionMappings).addParams(XmlHelper.getParams(actionElement))
   ...
   protected boolean verifyAction(String className, String name, Location loc) {
      ...
      try {
         ...
         throw new ConfigurationException("Action class [" + className + "] does not have a public no-arg constructor",
               e, loc);
   ...
   protected PackageConfig addPackage(Element packageElement) throws ConfigurationException {
      ...
      if (packageConfig != null) {
         if (LOG.isDebugEnabled()) {
            LOG.debug("Package [#0] already loaded, skipping re-loading it and using existing PackageConfig [#1]",
      ...
      for (int i = 0; i < resultTypeList.getLength(); i++) {
         ...
         if (clazz != null) {
            ...
            ResultTypeConfig.Builder resultType = new ResultTypeConfig.Builder(name, className).defaultResultParam(
                  paramName).location(DomHelper.getLocationObject(resultTypeElement));
   ...
   protected Class verifyResultType(String className, Location loc) {
      try {
         ...
         if (LOG.isWarnEnabled()) {
            LOG.warn("Result class [#0] doesn't exist (ClassNotFoundException) at #1, ignoring", e, className,
                  loc.toString());
         }
      } catch (NoClassDefFoundError e) {
         if (LOG.isWarnEnabled()) {
            LOG.warn("Result class [#0] doesn't exist (NoClassDefFoundError) at #1, ignoring", e, className,
                  loc.toString());
         }
      }
      return null;
         ...
         if (resultElement.getParentNode().equals(element)
            ...
            if (StringUtils.isEmpty(resultType)) {
               ...
               if (StringUtils.isEmpty(resultType)) {
                  ...
                  throw new ConfigurationException("No result type specified for result named '" + resultName
                        + "', perhaps the parent package does not specify the result type?", resultElement);
               }
            }
            ...
            if (config == null) {
               throw new ConfigurationException("There is no result type defined for type '" + resultType
                     + "' mapped with name '" + resultName + "'." + "  Did you mean '" + guessResultType(resultType)
                     + "'?", resultElement);
            }
   ...
   protected InterceptorStackConfig loadInterceptorStack(Element element, PackageConfig.Builder context)
      ...
      InterceptorStackConfig.Builder config = new InterceptorStackConfig.Builder(name).location(DomHelper
   ...
   protected void loadInterceptors(PackageConfig.Builder context, Element element) throws ConfigurationException {
      ...
      for (int i = 0; i < interceptorList.getLength(); i++) {
         ...
         InterceptorConfig config = new InterceptorConfig.Builder(name, className).addParams(params)
               .location(DomHelper.getLocationObject(interceptorElement)).build();
    ...
    */
   private List<InterceptorMapping> lookupInterceptorReference(PackageConfig.Builder context,
         Element interceptorRefElement) throws ConfigurationException {
