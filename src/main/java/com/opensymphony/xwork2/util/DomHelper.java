 ...
 * @since 1.2
 */
public class DomHelper implements Serializable {
      ...
      if (parserProp != null) {
         try {
            ...
            if (LOG.isErrorEnabled()) {
               LOG.error("Unable to load saxParserFactory set by system property 'xwork.saxParserFactory': #0", e,
                     parserProp);
            }
         } catch (Exception e) {
            if (LOG.isErrorEnabled()) {
      ...
      static {
         ...
         if (parserProp != null) {
            try {
               ...
               if (LOG.isErrorEnabled()) {
                  LOG.error(
                        "Unable to load SAXTransformerFactory set by system property 'xwork.saxTransformerFactory': #0",
                        e, parserProp);
               }
            } catch (Exception e) {
               if (LOG.isErrorEnabled()) {
