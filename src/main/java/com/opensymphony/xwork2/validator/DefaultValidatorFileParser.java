 ...
 */
public class DefaultValidatorFileParser implements ValidatorFileParser, Serializable {
   ...
   public List<ValidatorConfig> parseActionValidatorConfigs(ValidatorFactory validatorFactory, InputStream is,
      ...
      if (doc != null) {
         ...
         for (int i = 0; i < nodes.getLength(); i++) {
            ...
            try {
               ...
               objectFactory.buildValidator(className, new HashMap<String, Object>(), ActionContext.getContext()
                     .getContextMap());
   ...
   private void addValidatorConfigs(ValidatorFactory factory, NodeList validatorNodes, Map<String, Object> extraParams,
      ...
      for (int j = 0; j < validatorNodes.getLength(); j++) {
         ...
         ValidatorConfig.Builder vCfg = new ValidatorConfig.Builder(validatorType).addParams(params)
         ...
         if ((key != null) && (key.trim().length() > 0)) {
            ...
            vCfg.messageParams(sortedMessageParameters.values().toArray(
            ...
            if (messageParams != null && (messageParams.size() > 0)) {
               ...
               if (LOG.isWarnEnabled()) {
                  LOG.warn("validator of type ["
                        + validatorType
                        + "] have i18n message parameters defined but no i18n message key, it's parameters will be ignored");
               }
            }
         }
