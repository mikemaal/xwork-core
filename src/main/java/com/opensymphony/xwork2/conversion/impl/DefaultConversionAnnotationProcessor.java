 ...
 */
public class DefaultConversionAnnotationProcessor implements ConversionAnnotationProcessor, Serializable {
   ...
   public void process(Map<String, Object> mapping, TypeConversion tc, String key) {
      ...
      try {
         if (tc.type() == ConversionType.APPLICATION) {
            ...
            if (tc.rule() != ConversionRule.ELEMENT || tc.rule() == ConversionRule.KEY
                  || tc.rule() == ConversionRule.COLLECTION) {
               mapping.put(key, converterCreator.createTypeConverter(tc.converter()));
            } else //for keys of Maps
            if (tc.rule() == ConversionRule.KEY) {
